...
func TestCreateTestLessonHandler(t *testing.T) {
	t.Parallel()
	...
	t.Run("valid", func(t *testing.T) {
		...
		err = handler.Handle(ctx, &app.CreateTestLesson{
			ID:           lessonID,
			CourseID:     courseID,
			SectionID:    sectionID,
			Title:        "New Test",
			QuestionType: app.QuestionTypeMultipleChoice,
			Questions: []app.TestQuestion{
				{ID: uuid.New(), Question: "Q1", Answers: []app.TestAnswer{
					{ID: uuid.New(), Content: "A1", IsCorrect: true},
				}},
			},
			UserID: "user-1",
		})
