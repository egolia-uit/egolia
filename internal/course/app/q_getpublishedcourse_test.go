...
func TestGetPublishedCoursesHandler(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name: "courses found -> returns list (filtered)",
			setup: func(t *testing.T) *app.MockGetCoursesReadModel {
				t.Helper()
				mockRead := app.NewMockGetCoursesReadModel(t)
				mockRead.EXPECT().GetCourses(ctx, mock.MatchedBy(func(params *app.GetCourses) bool {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			mockRead := tc.setup(t)
			handler := app.NewGetPublishedCoursesHandler(mockRead)
