...
func New(
	...
	httpPing := fmt.Sprintf("http://%s/billing/ping", serverCfg.Health.Address())
	...
	mux.Handle("/billing/health/startup", health.NewHandler(startupChecker))
	mux.Handle("/billing/health/ready", health.NewHandler(readyChecker))
	mux.Handle("/billing/health/live", health.NewHandler(liveCheck))
	return &Health{
		&http.Server{
			Handler: mux,
