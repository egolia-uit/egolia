# Sequence DeclineCourse

:::info
Admin từ chối phê duyệt khóa học.
:::
...
A -> WA: Click "Decline" on pending course
activate A
activate WA
WA -> WA: Display decline dialog\nwith reason input

A -> WA: Enter decline reason
A -> WA: Confirm decline

WA -> GW: POST /admin/courses/{courseId}/decline\n{reason}
activate GW
GW -> GW: Validate JWT, check admin role
GW -> ACC: Forward request
activate ACC

ACC -> CA: DeclineCourse(courseId, adminID, reason)
activate CA
...
alt Course not found
  ...
  alt Course not pending
    ...
    CA -> C: Update status to declined
    activate C
    C -> C: Set status = 'declined'
    CA <-- C: Updated course
    deactivate C
    ...
    GW ->> NC: Async: Send decline notification with reason
    activate NC
    NC -> NC: Queue email with decline reason
    deactivate NC

    WA <-- GW: Decline success
    deactivate GW
...
<!-- diagram id="sequence-egolia-course-decline-course" -->
